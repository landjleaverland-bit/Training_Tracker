<script lang="ts">
	import { getCampusSessions } from '$lib/services/api';
	...
	async function loadSessions() {
		loading = true;
		try {
			const result = await getCampusSessions();
			if (result.ok && result.data) {
				// Format remote sessions
				sessions = result.data.map((r) => ({
					...r,
					activityType: 'campus_boarding' as const,
	...
	{:else}
		<div class="timeline">
			{#each filteredSessions.slice(0, visibleCount) as session}
				<CampusSessionCard {session} onDelete={handleSessionDeleted} />
			{/each}
