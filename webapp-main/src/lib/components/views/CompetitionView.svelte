					...
					{#if expandedDetails.has(session.id)}
						<div class="card-body" transition:slide={{ duration: 150 }}>
								...
								{#if editingNotes.has(session.id)}
									<div class="edit-notes-area" transition:slide={{ duration: 150 }}>
										...
										<div class="edit-actions">
											<button
												class="cancel-btn"
												onclick={() => cancelEditNotes(session.id)}
												disabled={savingNotes.has(session.id)}>Cancel</button
											>
											<button
												class="save-btn"
												onclick={() => saveNotes(session)}
												disabled={savingNotes.has(session.id)}
											>
													...
													{#each round.climbs || [] as climb}
														<div class="climb-row">
															...
															<div class="climb-result">
																<span
																	class="status-badge"
																	class:top={climb.status === 'Top' || climb.status === 'Flash'}
																	class:zone={climb.status === 'Zone'}
																>
																	{climb.status}
																</span>
															</div>
							...
							{#if session.fingerLoad || session.shoulderLoad}
								<div class="load-meta">
									<span
										>Detailed Load: Fingers {session.fingerLoad}/5, Shoulders {session.shoulderLoad}/5,
										Forearms {session.forearmLoad}/5</span
									>
								</div>
							{/if}
