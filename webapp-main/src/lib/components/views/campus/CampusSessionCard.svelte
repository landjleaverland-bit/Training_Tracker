<script lang="ts">
	import type { CampusSession } from '$lib/types/session';
	import { deleteCampusSession } from '$lib/services/api';
	...
	async function confirmDeleteSession() {
		try {
			const result = await deleteCampusSession(session.id);
			if (result.ok) {
	...
	function getTotalSets(s: CampusSession) {
...
<div class="session-card" class:expanded={isExpanded}>
	...
	{#if isExpanded}
		<div class="card-body" transition:slide={{ duration: 150 }}>
					...
					<div class="exercise-item">
						<div class="ex-header">
							<div class="ex-title-group">
								<span class="ex-name">{exercise.exerciseType}</span>
								<span class="ex-grip">{exercise.wallType}</span>
								{#if exercise.grade}
									<span class="ex-grip size-modifier">{exercise.grade}</span>
								{/if}
							</div>
						</div>
						<div class="ex-sets">
							{#each exercise.details as set}
								<span class="set-tag">
									{set.weight > 0 ? `+${set.weight}kg` : set.weight < 0 ? `${set.weight}kg` : 'BW'}
									<span class="x">x</span>
									{set.reps} reps
								</span>
							{/each}
			...
			{#if session.notes}
				<div class="session-notes-display">
					<h5>Notes</h5>
					<p>{session.notes}</p>
				</div>
			{/if}
